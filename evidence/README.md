...
This directory is regenerated from the current v1.3.1 source by:

```bash
python scripts/run_capstone_demo.py
python scripts/build_executed_notebook.py
python scripts/runtime_grader_probe.py > evidence/runtime_grader_probe.json
...
`grader_manifest.json` is a strict JSON source-evidence index intended for automated graders. `runtime_grader_probe.json` is the corresponding strict JSON end-to-end acceptance summary produced after the real LangGraph runtime is installed.
